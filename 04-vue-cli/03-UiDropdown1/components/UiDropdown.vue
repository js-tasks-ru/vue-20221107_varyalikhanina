<template>
  <div class="dropdown" :class="isMenuOpen ? 'dropdown_opened' : ''">
    <button
      type="button"
      class="dropdown__toggle"
      :class="optionHasIcon ? 'dropdown__toggle_icon' : ''"
    ...
    <div v-show="isMenuOpen" class="dropdown__menu" role="listbox">
      <button
        ...
        :class="optionHasIcon ? 'dropdown__item_icon' : ''"
        role="option"
        type="button"
        :value="item.value"
        @click="handleDropDown(item.value)"
      >
...
export default {
  ...
  computed: {
    icon() {
      if (this.modelValue) {
        const icon = this.options.filter((el) => el.value === this.modelValue)[0].icon;
        if (!icon) return;
        return icon;
      } else {
        return '';
    ...
    activeText() {
      if (this.modelValue) {
        const text = this.options.filter((el) => el.value === this.modelValue)[0].text;
        if (!text) return;
        return text;
      } else {
        return this.title;
